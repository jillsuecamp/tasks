...
export function isCorrect(question: Question, answer: string): boolean {
    const questionLowerCase: string = question.expected.toLowerCase().trim();
    const answerLowerCase: string = answer.toLowerCase().trim();
    if (questionLowerCase === answerLowerCase) {
        return true;
    } else {
        return false;
    }
...
export function toMarkdown(question: Question): string {
    ...
    if (question.type === "short_answer_question") {
        ...
        const questionBody: string = question.body;
        const options: string = question.options.join("\n- ");
        return firstLine + questionBody + "\n" + "- " + options;
    } else {
        return "";
    }
}
